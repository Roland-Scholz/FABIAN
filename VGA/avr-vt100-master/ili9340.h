...
#define ILI9340_WIDTH 80
#define ILI9340_HEIGHT 25

static uint8_t ili9340_color;

void ili9340_init(void) {
	printf_tiny("ili9340_init\n");
}
...
void ili9340_drawChar(uint16_t x, uint16_t y, uint8_t c) {
	printf_tiny("ili9340_drawChar %d %d %d %x\n", x, y, c, ili9340_color);
	uint8_t *mem = 0x8000;
	
	mem += (x + y * ILI9340_WIDTH) << 1;
...
void ili9340_setBackColor(uint16_t col) {
	printf_tiny("ili9340_setBackColor %d\n", col);
	
	ili9340_color &= 0xf0;
	ili9340_color |= (col & 0x0f);
	
}

void ili9340_setFrontColor(uint16_t col) {
	printf_tiny("ili9340_setFrontColor %d\n", col);
	ili9340_color &= 0x0f;
	ili9340_color |= (col & 0xf0);
}
...
void ili9340_fillRect(uint16_t x, uint16_t y, uint16_t w, uint16_t h,
  uint16_t color) {
	...
	printf_tiny("ili9340_fillRect %d %d %d %d %d \n", x, y, w, h, color);
	
	c = color << 4;
	
	for (i = y, i0 = y + h; i < i0; i++) {
		mem = 0x8000 + ((i * ILI9340_WIDTH) << 1);
...
void ili9340_setScrollStart(uint16_t start) {
	printf_tiny("ili9340_setScrollStart %d\n", start);
}

void ili9340_setScrollMargins(uint16_t top, uint16_t bottom) {
	printf_tiny("ili9340_setScrollMargins %d %d\n", top, bottom);
}
